...
import { extractApi, extractExamples, resolveComponentSource } from './showcase/api.ts';
...
function writeDocs(entry: ShowcaseMeta): void {
  ...
  const examples = extractExamples(entry.absolutePath, componentFile, entry.componentName);

  const contents = `// Generated by scripts/build-showcase-registry.ts - do not edit.
import type { ApiMember, ExampleSource } from '../../core/api';

export const html = ${JSON.stringify(html)};

export const api: readonly ApiMember[] = ${JSON.stringify(api, null, 2)};

export const examples: readonly ExampleSource[] = ${JSON.stringify(examples, null, 2)};
`;
